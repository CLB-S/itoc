...
public class ChunkManager
{
    ...
    private void UpdateNeighborMesherMasks(Chunk chunk)
    {
        ...
        if (
            Chunks.TryGetValue(
                new Vector3I(chunkIndex.X + 1, chunkIndex.Y, chunkIndex.Z),
                out var positiveXNeighbor
            )
        )
        {
        ...
        if (
            Chunks.TryGetValue(
                new Vector3I(chunkIndex.X, chunkIndex.Y + 1, chunkIndex.Z),
                out var positiveYNeighbor
            )
        )
        {
        ...
        if (
            Chunks.TryGetValue(
                new Vector3I(chunkIndex.X, chunkIndex.Y, chunkIndex.Z + 1),
                out var positiveZNeighbor
            )
        )
        {
            for (var x = 0; x < Chunk.SIZE; x++)
