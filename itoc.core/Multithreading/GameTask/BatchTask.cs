...
public class BatchTask : GameTask
{
    ...
    public BatchTask(
        ...
        params GameTask[] tasks
    )
        : base(name, priority) => _tasks = tasks ?? throw new ArgumentNullException(nameof(tasks));
    ...
    protected override void ExecuteCore(CancellationToken cancellationToken)
    {
        for (var i = 0; i < _tasks.Length; i++)
        {
            ...
            ReportProgress(
                (i * 100) / _tasks.Length,
                $"Executing task {i + 1} of {_tasks.Length}: {task.Name}"
            );
            ...
            {
                Godot.GD.PrintErr(
                    $"Error executing task {i + 1} of {_tasks.Length} in batch '{Name}': {ex.Message}"
                );
                throw;
            }
        }
