...
public static class PatternTreeJsonUtility
{
    ...
    {
        string json = PatternTreeJsonConverter.Serialize(node);
    ...
    public static void SaveToGodotResource(PatternTreeNode node, string filePath)
    {
        ...
        {
            GD.PrintErr($"Failed to save pattern tree to {filePath}. Error: {Godot.FileAccess.GetOpenError()}");
        }
    }
    ...
    public static PatternTreeNode LoadFromGodotResource(string filePath)
    {
        ...
        {
            GD.PrintErr($"Failed to load pattern tree from {filePath}. Error: {Godot.FileAccess.GetOpenError()}");
            return null;
        }
    }
    ...
    {
        string fileName = $"{patternTree.Id}.pattern";
    ...
    {
        PatternTreeNode node = LoadFromFile(filePath);
    ...
    {
        string fileName = $"{patternTree.Id}.tres";
        string filePath = Path.Combine(directory, fileName);
    ...
    {
        PatternTreeNode node = LoadFromGodotResource(filePath);
        if (node is PatternTree patternTree)
        {
            return patternTree;
        }
